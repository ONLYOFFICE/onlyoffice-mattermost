...
func (e *FilePersistenceError) Error() string {
	return fmt.Sprintf(onlyofficeFilePersistenceErrorPrefix+"file %s could not be saved: %s", e.FileID, e.Reason)
}
...
func (e *FileNotFoundError) Error() string {
	return fmt.Sprintf(onlyofficeLoggerCallbackPrefix+"file %s not found: %s", e.FileID, e.Reason)
}
