...
func (p *Plugin) forkRouter() *mux.Router {
	...
	subrouter.HandleFunc("/download", p.docServerOnly(p.downloadFile)).Methods(http.MethodGet)
	subrouter.HandleFunc("/editor", p.authenticationMiddleware(p.fileAuthorizationMiddleware((p.editor)))).Methods(http.MethodPost)
	subrouter.HandleFunc("/callback", p.docServerOnly(p.callback)).Methods(http.MethodPost)
